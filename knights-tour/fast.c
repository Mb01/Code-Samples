...
square* new_board(){
  ...
  for(int sq = 0; sq < DIM_SQUARED; sq ++){
    ...
    for(int mov = 0; mov < NUM_MOVES; mov++){
      ...
      int offset = xy_to_offset(KNIGHT_MOVES[mov][0] + cur_x, KNIGHT_MOVES[mov][1] + cur_y);
...
/* still pretty slow */
void knights_tour_corner(square* board, square* current_position, int depth)
{
  /* board pointer is retained for simple printing of solutions */
  /* current position is square from which to move */
  /* depth counts from one for marking move number */
...
void knights_tour_count(square* board, square* current_position, int depth)
{
  /* board pointer is retained for simple printing of solutions */
  /* current position is square from which to move */
  /* depth counts from one for marking move number */
